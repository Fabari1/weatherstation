# Internet of Things (IoT) Template - Weather Station and Analysis

### Description

The Hardware component has to monitor the environment, the hardware employs a DHT22 temperature and humidity sensor, a Soil Moisture Sensor, a BMP280 Pressure Sensor. The heat index must be calculated based on the sensor's data also the Soil Moisture Sensor should calibrated with a air and water value and then mapped to a range between zero to a hundred inclusive. The collected sensor data must be published to a topic subscribed to by both the backend and frontend segments of the system. This comprehensive functionality ensures seamless control, monitoring, and responsiveness within the environment.
